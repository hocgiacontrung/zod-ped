...
| 1b · SILVER trajectories | `01b_generate_silver.py` | RAN — **9,394 tracks**, cut+stitch ready, not poured |
| 2 · action labels (geometry) | `02_label_action.py` | DONE — all 11,257 tracks |
| 2e · human merge (GOLD) | `02e_merge_human_labels.py` | DONE — 217 human-verified → `actions_verified/` |
| 3 · samples + intent | `03_assemble_samples.py` | DONE on GOLD — **1,043 samples / 929 peds** |
| 4a · splits | `04_assign_splits.py` | **FROZEN** — 743/100/200 |
| 4b · packaging + QA | — | pending |

**Next:** (1) re-measure the geometry+PV-LSTM agreement rule on all 217 verified tracks
(`02d_build_review_queue.py --tier gold`), (2) pour SILVER through Steps 2–3, (3) reference baseline
on GOLD as a label sanity check.

**Open for the supervisor:** crossing ratio @2.0s is now **17.3%**, below the 20–30% target band —
the human pass removed geometry's false crossings. Relaxing the 50m `distance_to_ego` gate for
crosser windows is the main lever (458 windows lost there).
